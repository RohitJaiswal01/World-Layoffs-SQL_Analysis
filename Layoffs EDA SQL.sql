...
SELECT industry, SUM(total_laid_off) as laid_offs, avg(percentage_laid_off) as percentage_layoffs
FROM layoffs_staging2
GROUP BY industry
ORDER BY laid_offs DESC;
...
SELECT country, YEAR(date) AS year, industry, AVG(percentage_laid_off) AS avg_percentage_laid_off
FROM layoffs_staging2
WHERE percentage_laid_off IS NOT NULL
GROUP BY country, year, industry
ORDER BY year,country, avg_percentage_laid_off DESC;
...
-- Top  Companies with the Highest Layoff Percentage in Each Industry
...
-- Dataset ready for visualization 
 select count(distinct location), count(industry),count(distinct stage), count(distinct company), count(distinct country) from layoffs_staging2;
